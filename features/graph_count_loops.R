...
# This function returns the number of loops, i.e. transfers from one address to itself 
# Author: Bianca, added 11.4.2018


featureTable <- (function(tokenTransferTable, featureTable) {
  print("Computing the number of loops")
  ...
  feature <- tokenTransfers[, 
                            ...
                            list(graph_count_loops = sum(which_loop(graph_from_data_frame(.SD[, list(from, to)]))))
                            },
                            by = address]
  close(progressBar)
